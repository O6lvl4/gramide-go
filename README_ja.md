...
なかった破壊を「きれい」と数える([証拠](docs/evidence/recovery-goroot-src.json)、[仕組み](https://github.com/O6lvl4/gramide/blob/main/docs/recovery.md)):

| Go `src/`: 8,010 ファイル、30,927 回の破壊 | gramide | tree-sitter |
|---|---:|---:|
| 残った宣言(全破壊) | 99.8% | 91.1% |
| きれいに回復した破壊(壊した箇所以外を失わず、余計なものも出さない) | 99.3% | 82.9% |
| きれいに回復、`insert {` | 98.9% | 91.6% |
| きれいに回復、`delete }` | 99.9% | 48.2% |
| きれいに回復、`delete )` | 99.9% | 92.3% |
| きれいに回復、`insert (` | 98.8% | 97.0% |

4 通りすべてで gramide が上。差が大きいのは `}` を消した場合で、どの `{` が閉じ括弧を失ったかを
インデントが教え、そのブロックは同じ深さの次の行の手前で終わる。囲む関数も後続の関数も書かれた
とおりに読め、tree-sitter はコストモデルがファイルの残りを開いた本体に入れ子にする。
