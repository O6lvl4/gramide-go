...
|---|---:|---:|
| 残った宣言(全破壊) | 99.5% | 91.1% |
| きれいに回復した破壊(壊した箇所以外を失わず、余計なものも出さない) | 99.0% | 82.9% |
| きれいに回復、`insert {` | 98.8% | 91.6% |
| きれいに回復、`delete }` | 99.3% | 48.2% |
| きれいに回復、`delete )` | 99.5% | 92.3% |
| きれいに回復、`insert (` | 98.7% | 97.0% |

4 通りすべてで gramide が上。差が大きいのは `}` を消した場合で、gramide は次の `func` で
再開して失うのは関数 1 つ、tree-sitter はコストモデルがファイルの残りを開いた本体に入れ子にする。
